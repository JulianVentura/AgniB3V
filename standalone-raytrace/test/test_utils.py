...
def test_aparent_element_area_multiplier():
    ...
    aparent_area_multipliers = utils.aparent_element_area_multiplier(ray_directions, element_normals)
    expected_aparent_area_multipliers = [1, 1, np.sin(np.pi/4), np.sin(0.2657455355)]
    assert np.array_equal(np.floor(aparent_area_multipliers), np.floor(expected_aparent_area_multipliers))
